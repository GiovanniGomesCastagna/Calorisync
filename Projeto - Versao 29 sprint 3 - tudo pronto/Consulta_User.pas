unit Consulta_User;

interface

uses
  Winapi.Windows, Winapi.Messages, System.SysUtils, System.Variants,
  System.Classes, Vcl.Graphics,
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Data.DB, Vcl.Grids, Vcl.DBGrids,
  Vcl.StdCtrls, Vcl.ExtCtrls, Vcl.Mask, FireDAC.Stan.Intf, FireDAC.Stan.Option,
  FireDAC.Stan.Param, FireDAC.Stan.Error, FireDAC.DatS, FireDAC.Phys.Intf,
  FireDAC.DApt.Intf, FireDAC.Stan.Async, FireDAC.DApt, FireDAC.Comp.DataSet,
  FireDAC.Comp.Client, Conexao, System.StrUtils, Vcl.Imaging.pngimage;

type
  TConsulta_User = class(TForm)
    Nome_Razao_social: TLabel;
    Documentos: TLabel;
    Inserir_nome: TEdit;
    CNPJ: TRadioButton;
    CPF: TRadioButton;
    Inserir_CPF_CNPJ: TMaskEdit;
    Senha: TLabel;
    Digite_Senha: TEdit;
    CRN_Panel: TPanel;
    CRN_SIM_NAO: TLabel;
    Nao_CRN: TRadioButton;
    Sim_CRN: TRadioButton;
    Digite_CRN: TMaskEdit;
    Sxo_Panel: TPanel;
    Sexo: TLabel;
    Masculino: TRadioButton;
    Feminino: TRadioButton;
    FDQuery1: TFDQuery;
    DBGrid1: TDBGrid;
    Salvar: TButton;
    DataSource1: TDataSource;
    FDQuery1NOME: TWideStringField;
    FDQuery1CPF: TWideStringField;
    FDQuery1CNPJ: TWideStringField;
    FDQuery1TIPO_CONTA: TWideStringField;
    FDQuery1CNR: TWideStringField;
    FDQuery1SEXO: TWideStringField;
    Excluir: TButton;
    Image2: TImage;
    Image1: TImage;
    Image3: TImage;
    Image4: TImage;
    BtnNovo: TButton;
    btnCancelar: TButton;
    procedure FormCreate(Sender: TObject);
    procedure FormClose(Sender: TObject; var Action: TCloseAction);
    procedure FormKeyDown(Sender: TObject; var Key: Word; Shift: TShiftState);
    procedure FormShow(Sender: TObject);
    procedure ExcluirClick(Sender: TObject);
    procedure DBGrid1DblClick(Sender: TObject);
    procedure CNPJClick(Sender: TObject);
    procedure CPFClick(Sender: TObject);
    procedure SalvarClick(Sender: TObject);
    procedure Sim_CRNClick(Sender: TObject);
    procedure Nao_CRNClick(Sender: TObject);
    procedure AutoAjustarColunas(Grid: TDBGrid);
    procedure DBGrid1DrawColumnCell(Sender: TObject; const Rect: TRect;
      DataCol: Integer; Column: TColumn; State: TGridDrawState);
    procedure CarregarUsuarioPorDocumento(const Documento: string; IsCPF: Boolean);
    procedure Inserir_CPF_CNPJExit(Sender: TObject);
    procedure BtnNovoClick(Sender: TObject);
    procedure btnCancelarClick(Sender: TObject);
  private
    procedure OcultarColunaSenha;
    procedure LimparCampos;
    function CamposPreenchidos: Boolean;
  public
  end;

var
  FrmConsulta_User: TConsulta_User;

implementation

{$R *.dfm}

procedure TConsulta_User.FormCreate(Sender: TObject);
begin
  FDQuery1.Connection := DataModule1.FDConnection1;
end;

procedure TConsulta_User.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
  if Key = VK_ESCAPE then
    Close;
end;

procedure TConsulta_User.FormShow(Sender: TObject);
begin
  FDQuery1.Close;
  FDQuery1.SQL.Text :=
    'SELECT NOME, CPF, CNPJ, TIPO_CONTA, CNR, SEXO, SENHA FROM Usuarios';
  FDQuery1.Open;

  OcultarColunaSenha;

  FDQuery1CPF.EditMask := '000.000.000-00;0';
  FDQuery1CNPJ.EditMask := '00.000.000/0000-00;0';

  DBGrid1.Columns[3].Title.Caption := 'Tipo Conta';
  DBGrid1.Columns[6].Visible := False;
  AutoAjustarColunas(DBGrid1);
end;

procedure TConsulta_User.Nao_CRNClick(Sender: TObject);
begin
  Digite_CRN.Visible := False;
  Digite_CRN.Clear;
end;

procedure TConsulta_User.Sim_CRNClick(Sender: TObject);
begin
  Digite_CRN.Visible := True;
end;

procedure TConsulta_User.Inserir_CPF_CNPJExit(Sender: TObject);
begin
  if Trim(Inserir_CPF_CNPJ.Text) <> '' then
    CarregarUsuarioPorDocumento(Inserir_CPF_CNPJ.Text, CPF.Checked);
end;

procedure TConsulta_User.CNPJClick(Sender: TObject);
begin
  Inserir_CPF_CNPJ.EditMask := '00.000.000/0000-00;0';
  Inserir_CPF_CNPJ.Clear;
  Sxo_Panel.Visible := False;
end;

procedure TConsulta_User.CPFClick(Sender: TObject);
begin
  Inserir_CPF_CNPJ.EditMask := '000.000.000-00;0';
  Inserir_CPF_CNPJ.Clear;
  Sxo_Panel.Visible := True;
end;

procedure TConsulta_User.DBGrid1DblClick(Sender: TObject);
begin
  if FDQuery1.IsEmpty then
    Exit;
  Inserir_nome.Text := FDQuery1.FieldByName('NOME').AsString;
  if FDQuery1.FieldByName('CPF').AsString <> '' then
  begin
    CPF.Checked := True;
    Inserir_CPF_CNPJ.Text := FDQuery1.FieldByName('CPF').AsString;
  end
  else
  begin
    CNPJ.Checked := True;
    Inserir_CPF_CNPJ.Text := FDQuery1.FieldByName('CNPJ').AsString;
  end;
  if FDQuery1.FieldByName('SEXO').AsString = 'M' then
    Masculino.Checked := True
  else if FDQuery1.FieldByName('SEXO').AsString = 'F' then
    Feminino.Checked := True
  else
  begin
    Masculino.Checked := False;
    Feminino.Checked := False;
  end;
  if FDQuery1.FieldByName('CNR').AsString <> '' then
  begin
    Sim_CRN.Checked := True;
    Digite_CRN.Text := FDQuery1.FieldByName('CNR').AsString;
  end
  else
  begin
    Nao_CRN.Checked := True;
    Digite_CRN.Clear;
  end;
  Digite_Senha.Text := FDQuery1.FieldByName('SENHA').AsString;
end;

procedure TConsulta_User.DBGrid1DrawColumnCell(Sender: TObject;
  const Rect: TRect; DataCol: Integer; Column: TColumn; State: TGridDrawState);
var
  Texto: string;
begin
  if Column.Field.FieldName = 'SENHA' then
  begin
    Texto := StringOfChar('*', Length(Column.Field.AsString));
    DBGrid1.Canvas.FillRect(Rect);
    DBGrid1.Canvas.TextOut(Rect.Left + 2, Rect.Top + 2, Texto);
  end
  else
    DBGrid1.DefaultDrawColumnCell(Rect, DataCol, Column, State);
end;

procedure TConsulta_User.SalvarClick(Sender: TObject);
var
  CPF_CNPJ: string;
  IsCadastroNovo: Boolean;
  FDTemp: TFDQuery;
begin
  if (not CPF.Checked) and (not CNPJ.Checked) then
  begin
    ShowMessage('Selecione CPF ou CNPJ antes de salvar.');
    Exit;
  end;

  CPF_CNPJ := Trim(Inserir_CPF_CNPJ.Text);
  if CPF_CNPJ = '' then
  begin
    ShowMessage('Informe o CPF ou CNPJ.');
    Exit;
  end;

  FDTemp := TFDQuery.Create(nil);
  try
    FDTemp.Connection := FDQuery1.Connection;
    FDTemp.SQL.Text :=
      'SELECT COUNT(*) AS QTD FROM USUARIOS WHERE CPF = :DOC OR CNPJ = :DOC';
    FDTemp.ParamByName('DOC').AsString := CPF_CNPJ;
    FDTemp.Open;

    IsCadastroNovo := FDTemp.FieldByName('QTD').AsInteger = 0;
  finally
    FDTemp.Free;
  end;

  if IsCadastroNovo then
  begin
    FDQuery1.Connection.ExecSQL(
      'INSERT INTO USUARIOS (NOME, CPF, CNPJ, SEXO, CNR, SENHA, TIPO_CONTA) ' +
      'VALUES (:NOME, :CPF, :CNPJ, :SEXO, :CNR, :SENHA, :TIPO_CONTA)',
      [
        Inserir_nome.Text,
        IfThen(CPF.Checked, CPF_CNPJ, ''),
        IfThen(CNPJ.Checked, CPF_CNPJ, ''),
        IfThen(Masculino.Checked, 'M', IfThen(Feminino.Checked, 'F', '')),
        IfThen(Sim_CRN.Checked, Digite_CRN.Text, ''),
        Digite_Senha.Text,
        'J'
      ]
    );
    ShowMessage('Usuário cadastrado com sucesso!');
  end
  else
  begin
    FDQuery1.Connection.ExecSQL(
      'UPDATE USUARIOS SET ' +
      'NOME = :NOME, ' +
      'SEXO = :SEXO, ' +
      'CNR = :CNR, ' +
      'SENHA = :SENHA ' +
      'WHERE CPF = :DOC OR CNPJ = :DOC',
      [
        Inserir_nome.Text,
        IfThen(Masculino.Checked, 'M', IfThen(Feminino.Checked, 'F', '')),
        IfThen(Sim_CRN.Checked, Digite_CRN.Text, ''),
        Digite_Senha.Text,
        CPF_CNPJ
      ]
    );
    ShowMessage('Usuário atualizado com sucesso!');
  end;

  FDQuery1.Close;
  FDQuery1.SQL.Text :=
    'SELECT NOME, CPF, CNPJ, TIPO_CONTA, CNR, SEXO, SENHA FROM USUARIOS ORDER BY NOME';
  FDQuery1.Open;

  OcultarColunaSenha;
  AutoAjustarColunas(DBGrid1);
  LimparCampos;
end;

procedure TConsulta_User.ExcluirClick(Sender: TObject);
var
  CPF_CNPJ: string;
  Bookmark: TBookmark;
begin
  if FDQuery1.IsEmpty then
  begin
    ShowMessage('Nenhum registro selecionado!');
    Exit;
  end;

  if MessageDlg('Deseja realmente excluir este registro?', mtConfirmation,
    [mbYes, mbNo], 0) = mrNo then
    Exit;

  Bookmark := FDQuery1.GetBookmark;
  try
    if FDQuery1.FieldByName('CPF').AsString <> '' then
      CPF_CNPJ := FDQuery1.FieldByName('CPF').AsString
    else
      CPF_CNPJ := FDQuery1.FieldByName('CNPJ').AsString;

    FDQuery1.Connection.ExecSQL
      ('DELETE FROM Usuarios WHERE CPF = :CPF OR CNPJ = :CNPJ',
      [CPF_CNPJ, CPF_CNPJ]);

    FDQuery1.Close;
    FDQuery1.Open;

    OcultarColunaSenha;
    LimparCampos; // 🔹 limpa todos os campos após exclusão
    ShowMessage('Registro excluído com sucesso!');
  finally
    if FDQuery1.BookmarkValid(Bookmark) then
      FDQuery1.GotoBookmark(Bookmark);
    FDQuery1.FreeBookmark(Bookmark);
  end;
end;

procedure TConsulta_User.FormClose(Sender: TObject; var Action: TCloseAction);
begin
  Action := caFree;
  FrmConsulta_User := nil;
end;

procedure TConsulta_User.CarregarUsuarioPorDocumento(const Documento: string; IsCPF: Boolean);
var
  LSQL: string;
begin
  try
    FDQuery1.Close;
    FDQuery1.SQL.Clear;

    if IsCPF then
      LSQL :=
        'SELECT NOME, CPF, CNPJ, TIPO_CONTA, CNR, SEXO, SENHA ' +
        'FROM USUARIOS WHERE CPF = :DOC'
    else
      LSQL :=
        'SELECT NOME, CPF, CNPJ, TIPO_CONTA, CNR, SEXO, SENHA ' +
        'FROM USUARIOS WHERE CNPJ = :DOC';

    FDQuery1.SQL.Text := LSQL;
    FDQuery1.ParamByName('DOC').AsString := Documento;
    FDQuery1.Open;

    if not FDQuery1.Eof then
    begin
      Inserir_nome.Text := FDQuery1.FieldByName('NOME').AsString;
      Digite_Senha.Text := FDQuery1.FieldByName('SENHA').AsString;
      if IsCPF then
      begin
        if FDQuery1.FieldByName('SEXO').AsString = 'M' then
          Masculino.Checked := True
        else if FDQuery1.FieldByName('SEXO').AsString = 'F' then
          Feminino.Checked := True;
      end;
      if FDQuery1.FieldByName('CNR').AsString <> '' then
      begin
        Sim_CRN.Checked := True;
        Digite_CRN.Visible := True;
        Digite_CRN.Text := FDQuery1.FieldByName('CNR').AsString;
      end
      else
      begin
        Nao_CRN.Checked := True;
        Digite_CRN.Clear;
        Digite_CRN.Visible := False;
      end;
      // 🔸 Removido Salvar.Caption := 'Salvar';
    end;

    FDQuery1.Close;
    FDQuery1.SQL.Text :=
      'SELECT NOME, CPF, CNPJ, TIPO_CONTA, CNR, SEXO, SENHA FROM USUARIOS';
    FDQuery1.Open;

    OcultarColunaSenha;
    AutoAjustarColunas(DBGrid1);
  except
    on E: Exception do
      MessageDlg('Erro ao consultar documento: ' + E.Message, mtWarning, [mbOK], 0);
  end;
end;

procedure TConsulta_User.AutoAjustarColunas(Grid: TDBGrid);
var
  i: Integer;
  MaxLargura, LarguraTitulo, LarguraCampo: Integer;
  Texto: string;
begin
  if not Assigned(Grid.DataSource) or not Assigned(Grid.DataSource.DataSet) then
    Exit;

  for i := 0 to Grid.Columns.Count - 1 do
  begin
    LarguraTitulo := Grid.Canvas.TextWidth(Grid.Columns[i].Title.Caption) + 10;
    MaxLargura := LarguraTitulo;
    Grid.DataSource.DataSet.DisableControls;
    try
      Grid.DataSource.DataSet.First;
      while not Grid.DataSource.DataSet.Eof do
      begin
        Texto := Grid.Columns[i].Field.AsString;
        LarguraCampo := Grid.Canvas.TextWidth(Texto) + 35;
        if LarguraCampo > MaxLargura then
          MaxLargura := LarguraCampo;
        Grid.DataSource.DataSet.Next;
      end;
    finally
      Grid.DataSource.DataSet.EnableControls;
    end;
    Grid.Columns[i].Width := MaxLargura;
  end;
end;

procedure TConsulta_User.OcultarColunaSenha;
var
  i: Integer;
  Col: TColumn;
begin
  if DBGrid1.Columns.Count = 0 then Exit;

  for i := 0 to DBGrid1.Columns.Count - 1 do
  begin
    Col := DBGrid1.Columns[i];
    if SameText(Col.FieldName, 'SENHA') then
    begin
      Col.Visible := False;
      Exit;
    end;
  end;
end;

{ ======== NOVO E CANCELAR ======== }

procedure TConsulta_User.LimparCampos;
begin
  Inserir_nome.Clear;
  Inserir_CPF_CNPJ.Clear;
  Digite_Senha.Clear;
  Digite_CRN.Clear;
  CPF.Checked := False;
  CNPJ.Checked := False;
  Sim_CRN.Checked := False;
  Nao_CRN.Checked := False;
  Masculino.Checked := False;
  Feminino.Checked := False;
  Digite_CRN.Visible := False;
end;

function TConsulta_User.CamposPreenchidos: Boolean;
begin
  Result :=
    (Trim(Inserir_nome.Text) <> '') or
    (Trim(Inserir_CPF_CNPJ.Text) <> '') or
    (Trim(Digite_Senha.Text) <> '') or
    (Masculino.Checked or Feminino.Checked) or
    (Sim_CRN.Checked or Nao_CRN.Checked);
end;

procedure TConsulta_User.BtnNovoClick(Sender: TObject);
begin
  LimparCampos;
  ShowMessage('Pronto para inserir um novo usuário.');
  Inserir_nome.SetFocus;
end;

procedure TConsulta_User.btnCancelarClick(Sender: TObject);
begin
  if CamposPreenchidos then
  begin
    if MessageDlg('Deseja realmente cancelar a operação atual?',
      mtConfirmation, [mbYes, mbNo], 0) = mrNo then
      Exit;
  end;
  LimparCampos;
  ShowMessage('Operação cancelada.');
end;

end.

