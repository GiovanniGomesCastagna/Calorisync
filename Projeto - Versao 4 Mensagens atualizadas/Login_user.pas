...
type
  TForm2 = class(TForm)
    ...
    Fundo_Panel: TImage;
    Usuáro: TLabel;
    User_Name: TEdit;    // seu TEdit para CPF
    Senha: TLabel;
    Senha_User: TEdit;   // seu TEdit para senha
    Entrar: TButton;     // seu botão Entrar
    Imagem_Fundo: TImage;
    FDQuery1: TFDQuery;
    Fechar: TButton;
...
var
  PosCursor: Integer;
begin
  PosCursor := User_Name.SelStart;             // salva a posição do cursor
  User_Name.Text := UpperCase(User_Name.Text); // converte para maiúsculo
  User_Name.SelStart := PosCursor;             // restaura a posição do cursor
end;
...
begin
  ...
  begin
    Key := 0; // evita beep
    Senha_User.SetFocus; // vai para o campo de senha
  end;
end;
...
begin
  ...
  begin
    Key := 0; // evita beep
    EntrarClick(Sender); // executa o login
  end;
end;
